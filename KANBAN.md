...
- [ ] **Safari 上重跑 G-05 的逐字坐标比对**：行尾拆分方案只在 Chrome 152 验证过，
      ...
      **挂起，不阻塞 G-07**：没有 Mac，现在验不了。见文末「🔍 已知盲区」
- [ ] 首字延迟 P90 ≤2.5s（G-06 已实测 P90 682ms）
- [ ] **前端按 3–5 字节奏渲染**（从 G-06 移来：服务端切块正确，传输层会合并，
      视觉节奏只能由前端保证）
- [ ] **流式生成时正在读的内容不动**（产品不变量的直接推论）：
...
- [ ] 断网时点击句子有明确提示，非无限加载
- [ ] 中途断流（D2）：已收到的文字保留在撑开区，一行灰字注明没写完（不写「出错了」这类把责任推给用户的话），
...
- [ ] 同一句在本次会话内重复点击，不重复发请求（跨会话缓存是 G-09）
- [ ] 结构摘要：开书时后台计算一次，按 docId 缓存；算好之前的点击不带摘要照常发，不让第一次点击等待
...
- [ ] ⚠️ **公开链接的滥用防护**：PRD 3.8 的每日额度存在 localStorage，清一下就绕过。
      ...
      账户里常态保持 ≤¥20
- [ ] 🖐 **需人工操作（Clara 在 Vercel 控制台完成，不写代码）**：Firewall → 新建 WAF 限流规则——
