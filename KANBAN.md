...
- [x] **b. 一句白话里的标记不超过 3 处**：超过时保留前 3 处，其余按纯文本显示。标记的价值在稀少，满屏灰蓝等于没标
      （Claude Code 自测）：`MAX_TERMS_PER_GLOSS = 3`，`GlossPanel` 渲染时生效；没通过逐字校验的不占名额；
      同一个词出现几次数几次；相邻的两个术语（⟦实体⟧⟦样式⟧）分开计数。单元测试锁住
- [ ] **c. 「标得够不够、有没有标错」由 Clara 在第二批 10 句上判断**。她的判断就是标准；判定过程留痕：
      哪句、她认为该标哪几个、模型标了哪几个。**阻塞于第二批 10 句**
- [ ] Clara 亲验：三套主题下肉眼确认术语标记看得出来（术语色 vs 白话色只有 1.23–1.31:1，不是 WCAG 项，但可能太淡）
