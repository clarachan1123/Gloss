...
- [ ] **20 句评测集上术语识别准确率 ≥85%**：**未达标，且判定要等第二批 10 句**。
      三轮实测（见下方「术语识别实测」）召回率 3.9% → 3.9% → 7.8%，精确率 100% → 33.3% → 57.1%。
      现有 20 句只作参考数字（这批句子参与过 G-06 的 prompt 调整，有过拟合风险）
...
分母 51–53（Clara 在评测集里标注的术语，20 句里 17 句有标注）。三轮都没有出现定界符未配对。
...
需要 Clara 判断：评测集的「术语」字段标的是「原句里的难词」，还是「白话里应该被圈起来的词」。
